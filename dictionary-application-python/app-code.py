#Version 1.2 of the Dictionary-App
...
def translate(w):
    ...
    #convert the user input word in all lower case
    w = w.lower()

    #condition 1 : if the user input word is found as it is as one of the key in data dictionary then return the value of that pair
    if w in data:
        return data[w]
    ...
    #condition 3 : if the user input word is in all capital letters in the dictionary then this shoulf be the action.
    elif w.upper() in data:
        return data[w.upper()]

    #Condition 4 : if the value entered is close enough to one of the existing keys in data dictionary then ask the user if they meant that key instead
    ...
    #Condition 5 : if there is no match then tell the user that the word is not in this dictionary
    else:
        return "Meaning of this word dosent exist in this dictionary !!"
